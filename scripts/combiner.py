...
def combine_data(directory, base_name, ext, remove_files=True):
    ...
    if len(files_to_combine) == 0:
        logger.info("did not find any files to combine with %s", file_pattern)
        return
    
    if len(files_to_combine) != s.narray:
        logger.fatal("Found %d files to combine, expected %d", len(files_to_combine), s.narray)
        return
...
if __name__ == "__main__":
    ...
    if not os.path.isfile(s.patch_file):
        logger.info(f"Combining data files {s.patch_str} in {s.patch_dir}")
        combine_data(s.patch_dir, s.patch_str, ".hdf5")
    else:
        logger.info(f"Found completed patch file, skipping patch combination")
