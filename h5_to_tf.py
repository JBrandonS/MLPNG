...
# Copyright 2017
# taken from:  https://github.com/risteon/hdf5-to-tfrecord
# ==============================================================================
...
def convert_to(directory, dataset_name):
    ...
    files = sorted([os.path.join(directory, file)
                   for file in os.listdir(directory) if file.endswith('.hdf5')])
    ...
    with tf.python_io.TFRecordWriter(filename) as writer:
        ...
        print("Unique values in dataset '{}': {}".format(
            dataset_name, unique_values))
    ...
    def is_valid_folder(x):
        ...
        if not os.path.isdir(x):
            raise argparse.ArgumentTypeError(
                "{0} is not a directory".format(x))
        return x

    parser = argparse.ArgumentParser()
    parser.add_argument('--train_dir', metavar='directory',
