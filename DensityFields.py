...
class DensityField2D():
    ...
    def __init__(self, BoxSize, grid, 
                 n_threads=1, FFTW_WISDOM='FFTW_ESTIMATE', dtype=np.float32, 
                 cosmo_params=default_cosmo_params, camb_params_obj=None, init_camb=True,
                 qs = None, transfers = None, ls=None, ells = None, d_A = None, # will only be used if init_camb = False
                 logger=None, log_level=logging.INFO, verbose=False):
        ...
        self.kmin = np.min(self.kgrid)
        self.kmax = np.max(self.kgrid)
        ...
        if camb_params_obj is None and init_camb: 
            self.camb_params = camb_params_obj if camb_params_obj is not None else camb.CAMBparams()
        ...
        if init_camb:
            ...
            self.log.debug('Not running CAMB. Using provided transfer functions and ell values.')
        ...
        if verbose: self.log.info('Finished generating CMBField with resolution {}, box size {} Mpc/h, and cell size {}'.format(self.grid, self.BoxSize, self.cell_size)) 
        if verbose: self.log.info('min k: {}, max k: {}. kF: {}, kNyq: {}'.format(self.kmin, self.kmax, self.kF, self.kNyq))
    
    def run_camb(self, params=None):
        if params is None: 
            self.log.debug('Running CAMB with global parameters...')
            params = self.camb_params
        else: self.log.debug('Running CAMB with provided parameters {}...')
    ...
    def Load_r2c(self,delta_r):
        assert (delta_r.shape == self.r_delta.shape), self.log.fatal("mismatching grid sizes")
    ...
    def Load_c2r(self,delta_c):
        assert (delta_c.shape == self.c_delta.shape), self.log.fatal("mismatching grid sizes")
    ...
    def Pk(self,kmax=None):
        if kmax is None: kmax = self.kNyq
    ...
    def _Bk_counts(self,fc,dk,NBmax,triangle_type,data_dir='data/static',verbose=False):
        ...
        if os.path.exists(file_name):
            if verbose: self.log.info(f"Loading Counts from {file_name}")
            counts = np.load(file_name,allow_pickle=True).item()
            if verbose: self.log.info(f"Considering {len(counts['bin_centers'])} Triangle Configurations ({triangle_type})")
            return counts
        ...
        if verbose: self.log.info(f"Creating Grids for Counts...")
        ...
        if verbose: self.log.info("Computing Powerspectrum Counts...")
        counts['counts_P'] = _Pk_shells(r_ones_shells) * self.grid**2
        if verbose: self.log.info("done!")

        if verbose: self.log.info("Computing Triangle Counts...")
        ...
        if verbose: self.log.info(f"Saved Triangle Counts to {file_name}")
    ...
    def Bk(self,fc,dk,NBmax,triangle_type='All',verbose=False):
        ...
        r_delta_shells = np.zeros((NBmax,self.rshape[0],self.rshape[1]),dtype=self.r_dtype)

        if verbose: self.log.info(f"Creating Grids for Measurements...")
        for i in tqdm(range(NBmax),disable= not verbose):
            k_low = self.kF * (fc + dk * i - dk/2)
            k_high= self.kF * (fc + dk * i + dk/2)
            r_delta_shells[i] = self.mask_c2r(self.c_delta,k_low,k_high)

        if verbose: self.log.info(f"Computing Powerspectrum...") 
        P = _Pk_shells(r_delta_shells) * self.BoxSize**3 / counts['counts_P'] / self.grid**2
        if verbose: self.log.info("done!")

        if verbose: self.log.info(f"Computing Bispectrum...")    
        bin_indices = ((counts['bin_centers'] - fc) // dk).astype(np.int64)
    ...
    def GenerateLinearField(self,zi,k_cut_low=None,k_cut_high=None,fnl=0.,seed=0,verbose=False):
        ...
        if verbose: self.log.info("Applying primordial power spectrum...")
        ...
        if verbose: self.log.info("Applying linear transfer function...")
        apply_linear_power_or_transfer(self.c_fftgrid,self.kgrid,\
                                           kLin,TFLin,self.BoxSize,self.grid)
        if verbose: self.log.info("done!")
    ...
    def GenerateCAMBField(self,k_cut_low=None,k_cut_high=None,fnl=0.,seed:Optional[int]=0,verbose=False,force_camb_run=False,debug_plots=False):
        ...
        # Cut-off beyond Nyquist Frequency
        self.c_fftgrid[self.kgrid > self.kNyq] = 0.+0.j
        if debug_plots: plot_fields(np.abs(self.c_fftgrid), 'C -Nyquist', norm=LogNorm())
        ...
        if self.transfer is None or force_camb_run:
            if verbose: self.log.info("Generating CAMB results...")
            self.run_camb()
            if verbose: self.log.info("done!")
            
        if verbose: self.log.info("Applying camb transfer function...")
        apply_camb_power_or_transfer(self.c_fftgrid, self.kgrid, self.Ls, self.qs, self.transfers, self.ells, self.h, self.c_dtype) # type: ignore
        if verbose: self.log.info("done!")
...
## make CAMB transfer function
# @njit(parallel=True)
def apply_camb_power_or_transfer(delta_c, kgrid, ls, qs, transfers, ells, h, dtype = np.complex128):
    field = np.zeros_like(kgrid, dtype=dtype)
    for i in prange(kgrid.shape[0]):
        indices = np.array([find_closest_index(k*h, qs) for k in kgrid[i]])
        mask = (indices > 0) & (ells[i] >= 2)
        for j in range(kgrid.shape[1]):
            if mask[j]: field[i, j] = transfers[indices[j]](ells[i, j])
        delta_c[i] *= field[i]
...
def plot_fields(field, title=None, cmap='viridis', norm:Optional[Normalize]=None):
    ...
    # plt.imshow(field, cmap='RdBu_r', norm=norm)
