...
- `fnl_range`, [-100, 100] : Range of fNL values to use in the KSW estimator.
...
- `save_alms`, True : Boolean to save the alm data. If false the code will not save the alm data.
- `isotropic`, True : Boolean to set the sims to be isotropic.
...
- `--force_ksw` : Force the creation of the KSW MC data.
- `--double_precision` : Force double precision.
- `--plot` : Force generation of plots.
- `--isotropic` : Make the data isotropic.

If scripting runs you can save the settings with:

- `--save_settings` : Save the settings to a file, including CLI changes. This will save to a file in the `base_dir/runs` directory.
