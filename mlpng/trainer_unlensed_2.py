...
# Enable mixed precision (mixed_bfloat16 for stability on modern GPUs)
...
class UnifiedHead(tf.keras.layers.Layer):
    """Unified feedforward head with single shared encoder path and per-shape output layers.

    Architecture:
        Shared encoder: [256, 256, 128, 128, 64, 64, 32, 32] (8 deep layers)
        Per-shape outputs: Each shape gets a dedicated single-neuron output layer

    This approach:
        - Reduces overfitting by sharing learned features across shapes
        - Simpler than shape-specific heads
        - Still maintains per-shape output precision via dedicated output neurons
    """

    # Deeper shared encoder architecture (8 layers total)
    SHARED_ENCODER_ARCH = [128, 128, 128, 128, 64, 64, 64, 32]
    ...
    def __init__(self, n_outputs, task_names, activation="relu", dropout=0.0, **kwargs):
        ...
        self.n_shapes = len(self.task_names)
        ...
        self.encoder = []
        if dropout > 0:
            self.encoder.append(Dropout(dropout, name="encoder_dropout_0"))

        for i, size in enumerate(self.SHARED_ENCODER_ARCH):
            self.encoder.append(
                Dense(
                    size,
        ...
        # Build per-shape output layers (single neuron per shape)
        self.shape_outputs = []
        for shape_idx, shape_name in enumerate(self.task_names):
            self.shape_outputs.append(
                Dense(1, kernel_initializer=init, name=f"output_{shape_name}")
            )
    ...
    def call(self, x, training=False):
        ...
        for i, layer in enumerate(self.encoder):
            if isinstance(layer, Dropout):
                encoder_out = layer(encoder_out, training=training)
            else:
                encoder_out = layer(encoder_out)

        # Apply per-shape output layers and concatenate
        outputs = []
        for output_layer in self.shape_outputs:
            outputs.append(output_layer(encoder_out))

        return tf.keras.layers.Concatenate()(outputs) if outputs else encoder_out
...
def build_deep_task_model(
    ...
    """Build deep encoder with double-conv blocks, progressive K, and unified head.
    ...
    # Unified head with shared encoder and per-shape outputs
    x = Flatten()(x)
    outputs = UnifiedHead(
        n_outputs=n_outputs,
    ...
    model = tf.keras.Model(inputs, outputs, name="deep_task_encoder_unified")
    return model
...
def create_sigma_weighted_loss(sigma_all):
    ...
    Wrapped with @tf.function for XLA compatibility.
    """
...
class NeoTrainer:
    ...
    def train(
        ...
        if use_wandb:
            wandb.init(
                ...
                config={
                    ...
                    "model_type": "deep_task_unified",
                    "K_schedule": K_schedule,
                    "unified_head_architecture": UnifiedHead.SHARED_ENCODER_ARCH,
                    ...
                    "jit_compile": False,  # Disabled: DeepSphere uses sparse ops incompatible with XLA
                },
                tags=[
        ...
        with self.strategy.scope():
            ...
            # Compile model
            # Note: jit_compile=False because DeepSphere uses sparse tensor operations
            # (SparseTensorDenseMatMul) which are not supported by XLA on GPU
            self.model.compile(
                optimizer=optimizer,
        ...
        checkpoint_name = f"n{self.core.nside}_unlensed_2_{self.core.shapes_str}.h5"
...
def main():
    """Command-line interface for the deep trainer (unified version).
    ...
    """
    setup_logging("mlpng.trainer_unlensed_2", level=logging.DEBUG)
