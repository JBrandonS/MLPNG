...
def get_model(input_shape, max_batch_size=32, n_out=1):
    ...
    layers.append(Dense(512, activation=LeakyReLU(0.3)))
    layers.append(Dense(128, activation=LeakyReLU(0.3)))
...
def main():
    ...
    with strategy.scope():
        ...
        learning_rate = CosineDecayRestarts(
            learning_rate, decay_steps, t_mul=2.0, m_mul=1.0, alpha=0.1
        )

        model = get_model((None, core.npix, core.npols), batch_size, len(shapes))
        model.compile(
            optimizer=AdamW(learning_rate, weight_decay=0.01),  # type: ignore
            loss=irmse(),
