...
class NeoTrainer:
    ...
    def setup(self):
        ...
        ds = KappaDataset.fromCore(
            self.core, phi_scale=self.core.phi_scale, x_output="lensed", y_output="fnl"
        )

        # Create unique cache filename with nside and shapes to avoid lockfile conflicts
        cache_file = os.path.join(
            self.cache_dir,
            f"n{self.core.nside}_lensed_{self.core.shapes_str}_d{'_'.join(map(str, [25, 10, 2]))}_p{self.core.phi_scale}",
        )
    ...
    def train(
        ...
        checkpoint_name = f"n{self.core.nside}_lensed_{self.core.shapes_str}_p{self.core.phi_scale}.h5"
    ...
    def _plot_evaluation_base(
        ...
        for shape_idx, shape_name in enumerate(self.core.shapes):
            ...
            axes[0, shape_idx].plot(line, line - sigma, "g--")
            axes[0, shape_idx].set_xlabel(r"True $f_\mathrm{nl}$")
            axes[0, shape_idx].set_ylabel(r"Predicted $f_\mathrm{nl}$")
            axes[0, shape_idx].set_title(
                f"Lensed {shape_name} (N={sample_count}, RMSE: {rmse:.2f})"
            )
