...
2. Create your model
   ...
   - The model should be a class that inherits from `scripts.models.ModelCore`, needs a `_model` method that takes in at least the inputs and which outputs the final layer, and must be registered with `@scripts.models.register_model`
   - See `scripts/models/isensee.py` for a standalone example which can also be manually ran.
...
> [Weights and Biases](https://wandb.ai/) can be used for logging and tracking the training, similar to an online tensorboard with a few extra features. You will need to setup an account and install the `wandb` package to use this feature. You can enable this in the `training.py` file.
...
## Problems
 ...
 - The tuning code is not great, and probably wont be improved much as it isn't that useful.
